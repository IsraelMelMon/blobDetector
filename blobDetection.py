    ...
    def reseting(resetToggle):
        ...
        if resetToggle == True:
            ...
            for k in keypoints:
                a = np.float32(k.pt[0])
                b = np.float32(k.pt[1])
                c = np.float32(k.size/2)
                print(a,b,c)
                cv2.circle(img, (a, b), c, (0, 0, 255), -1)
                print(k.pt, k.size/2)
                cv2.putText(img,"r:"+ str(round(k.size/2,4)), (int(k.pt[0]), int(k.pt[1])), font, fontScale,fontColor,lineType)
        else:
            img = cv2.imread(path)
            keypoints = []
            #params.minArea = 0
...
if __name__ == "__main__":
    path = "circles/holes_8.jpeg"
    simpleBlobDetection(path)
    pass
